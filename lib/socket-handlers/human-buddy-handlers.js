      ...
      try {
        ...
        if (sessionId && userId) {
          ...
          if (sessionDoc.exists) {
            ...
            console.log(`[disconnect] Removing ${username} (${userId}) from session ${sessionId}`);
            console.log(`[disconnect] Participants before: ${participants.length}, after: ${updatedParticipants.length}`);

            // Update session document
            const updateData = {
              participants: updatedParticipants,
              participantCount: updatedParticipants.length,
              [`participants_${userId}`]: firebaseAdmin.firestore.FieldValue.delete(),
              status: updatedParticipants.length === 0 ? BUDDY_SESSION_STATUS.ABANDONED : sessionData.status,
              updatedAt: new Date(),
            };
            ...
            buddyNamespace.to(`buddy_${sessionId}`).emit('user_left_session', {
              userId,
              username,
              remainingParticipants: updatedParticipants.length,
            });

            console.log(`[disconnect] ✅ ${username} successfully removed from session ${sessionId}`);
          }
        }
      } catch (error) {
        console.error('[disconnect] ❌ Error removing user from session:', error);
      }
    });
  });
